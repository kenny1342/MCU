...
void doDecision(void)
{
    ...
    if(Status.freezer_running && (Temps.freezer_real > -33 && !Status.mode_deepfreeze)) // safety, real -35=-28 i skapet, etter kjørt 240 min...
    ...
    if(Status.run_freezer) // None of the above checks changed the default YES, let's do the temp checks
    {
        ...
        {
            if(Temps.freezer_calc <= (Temps.pot_freezer + FREEZER_DEGREES_OVER)) // shall we start?
            {
                Status.run_freezer = NO;                
            }
        }        
    ...
    if(Status.fridge_running) // None of the above checks changed the default YES, let's do the temp checks
    {
    ...
    {
        if(Temps.fridge_calc <= (Temps.pot_fridge + FRIDGE_DEGREES_OVER)) // shall we start?
        {
            Status.run_fridge = NO;                
        }
    }        
