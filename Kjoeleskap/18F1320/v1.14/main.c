...
void doDecision(void)
{
    ...
    if( (Temps.fridge_real == 0 && Temps.freezer_real == 0) || Temps.fridge_real == NOT_READY || Temps.freezer_real == NOT_READY || Temps.pot_fridge == NOT_READY || Temps.pot_freezer == NOT_READY) 
    ...
    if(Status.freezer_running && (Temps.freezer_real > -33 && !Status.mode_fastfreeze)) // safety, real -35=-28 i skapet, etter kjørt 240 min...
...
void updateAlarms(void)
{
    ...
    //if((Temps.fridge_real + config.fridge_stop_temp_offset) > FRIDGE_MAX_TEMP) Temps.fridge_to_warm = YES; else Temps.fridge_to_warm = NO;    
    //if( (Temps.fridge_real + config.fridge_run_temp_offset) < FRIDGE_MIN_TEMP) Temps.fridge_to_cold = YES; else Temps.fridge_to_cold = NO;

    if(Temps.fridge_calc > (Temps.pot_fridge + FRIDGE_DEGREES_OVER)) Temps.fridge_to_warm = YES; else Temps.fridge_to_warm = NO;    
    if(Temps.fridge_calc < (Temps.pot_fridge - config.fridge_degrees_under)) Temps.fridge_to_cold = YES; else Temps.fridge_to_cold = NO;
