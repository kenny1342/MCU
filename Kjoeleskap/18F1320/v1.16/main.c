...
{    
    ...
    if((Temps.fridge_real + (config.fridge_stop_temp_offset-1)) > (Temps.pot_fridge + FRIDGE_DEGREES_OVER)) Temps.fridge_to_warm = YES; else Temps.fridge_to_warm = NO;            
    if(Temps.fridge_real < FRIDGE_MIN_TEMP) Temps.fridge_to_cold = YES; else Temps.fridge_to_cold = NO;
    
    // Update FREEZER temp struct        
    if((Temps.freezer_real + (config.freezer_stop_temp_offset-1)) > (Temps.pot_freezer + FREEZER_DEGREES_OVER)) Temps.freezer_to_warm = YES; else Temps.freezer_to_warm = NO;
    ...
    if(Alarms.fridge_run_to_long)
        #ifdef DEBUG printf(STREAM_DEBUG, "**A: Freezer run to long\r\n");  #endif
