...
void readPotFreezer(void)
{
    ...
    {                
        ...
        //adc_val += 30L; // compensate for adjusted VDD
        
        SampleFreezerPot.index = 0;        

        SampleFreezerPot.avg_adc_value = adc_val;
        
        if(adc_val > PotFreezerLevels[7]) Temps.pot_freezer = POT_OFF;
        else if(adc_val > PotFreezerLevels[6]) Temps.pot_freezer = -18;
        else if(adc_val > PotFreezerLevels[5]) Temps.pot_freezer = -19;
        else if(adc_val > PotFreezerLevels[4]) Temps.pot_freezer = -20;
        else if(adc_val > PotFreezerLevels[3]) Temps.pot_freezer = -21;
        else if(adc_val > PotFreezerLevels[2]) Temps.pot_freezer = -22; // 400
        else if(adc_val > PotFreezerLevels[1]) Temps.pot_freezer = -23;    //340
        else if(adc_val > PotFreezerLevels[0]) Temps.pot_freezer = -24;
        else Temps.pot_freezer = -25;

        if(adc_val <PotFreezerLevels[8]) Temps.pot_freezer = POT_OFF;        
...
void readPotFridge(void)
{
    ...
    {                
        ...
        if(adc_val < PotFridgeLevels[6]) Temps.pot_fridge = POT_OFF; // POT er litt knirkete...
        else if(adc_val > PotFridgeLevels[5]) Temps.pot_fridge = POT_OFF;
        else if(adc_val > PotFridgeLevels[4]) Temps.pot_fridge = 6;
        else if(adc_val > PotFridgeLevels[3]) Temps.pot_fridge = 5;
        else if(adc_val > PotFridgeLevels[2]) Temps.pot_fridge = 4;
        else if(adc_val > PotFridgeLevels[1]) Temps.pot_fridge = 3;        
        else if(adc_val > PotFridgeLevels[0]) Temps.pot_fridge = 2;
        else
            Temps.pot_fridge = 1;
