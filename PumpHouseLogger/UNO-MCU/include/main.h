...
#define CORR_FACTOR_PRESSURE_SENSOR 30  // correction factor (linear) (in Bar*100)
#define ADC_CH_WATERP               1 // ADC connected to water pressure sensor
#define ADC_CH_TEMP_1               0 // ADC connected to LM335 temp sensor 1
#define ADC_SAMPLE_COUNT            5 // nr of ADC readings to calculate average from

typedef struct
{
  uint16_t adc_temp_1;
  uint16_t adc_waterpump;
  char temp_pumphouse[4];
